 ...
 * `/yimark/api/fetch?url=…`：生产环境由 nginx 反代实现（只放行
 * mp.weixin.qq.com），开发环境由 vite.config.ts 里的同名插件实现，
 * 两边只做转发，解析全部在前端完成。
 *
 * 微信文章的两个关键点：
...
const PROXY_PATH = '/yimark/api/fetch?url=';
...
export async function importWechatArticle(rawUrl: string): Promise<WechatArticle> {
  ...
  const res = await fetch(PROXY_PATH + encodeURIComponent(url));
